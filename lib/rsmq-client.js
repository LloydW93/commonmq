...
const RSMQClient = (function() {
    ...
    RSMQClient.prototype = {
        send: function(message, callback) {
            this._rsmq.sendMessage({qname: this.queueId, message: message}, callback);
        },
        receive: function(callback) {
            let client = this;
            client._rsmq.receiveMessage({qname: client.queueId}, function(err, msg) {
                if (err || msg) {
                    callback(err, msg ? {
                        id: msg.id,
                        data: msg.message
        ...
        badMessage: function(messageId, message, callback) {
            let client = this;
            client._rsmq.sendMessage({qname: client.badMessageQueueId, message: message}, function(err, data) {
                if (!err) {
